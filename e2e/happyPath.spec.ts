  ...
  test('launch → configure → index → search → open', async () => {
    ...
    try {
      ...
      const statusBefore = await page.evaluate(async () => {
        return await (
          window as unknown as { kordaAPI: { fileIndexStatus(): Promise<unknown> } }
        ).kordaAPI.fileIndexStatus()
      })
      ...
      // 4. Add a new source with the test data root path
      ...
      await expect(page.locator('text=Source saved')).toBeVisible({ timeout: 5_000 })
      ...
      await expect(page.locator('text=Index ready'))
        ...
        .catch(async () => {
          ...
          await expect(
            page
              .locator('[class*="surface-raised"] span, .text-xs span')
      ...
      await expect(
        page.locator('[data-testid="search-result-item"] >> text=C-101').first(),
      ).toBeVisible()

      // 14. Inject shell.openPath spy
      type AugmentedShell = Electron.Shell & {
        __lastOpenedPath: string | null
        __origOpenPath: (path: string) => Promise<string>
      }
      await app.evaluate(({ shell }) => {
        const s = shell as unknown as AugmentedShell
        s.__lastOpenedPath = null
        s.__origOpenPath = shell.openPath.bind(shell)
        shell.openPath = async (p: string) => {
          s.__lastOpenedPath = p
          return ''
        }
      })
      ...
      const openedPath = await app.evaluate(
        ({ shell }) => (shell as unknown as AugmentedShell).__lastOpenedPath,
      )
      ...
      await app.evaluate(({ shell }) => {
        const s = shell as unknown as AugmentedShell
        shell.openPath = s.__origOpenPath
      })
    } finally {
